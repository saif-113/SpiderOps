...
const existingIncidents = database
...
if (existingIncidents.count === 0) {
  ...
  const seedIncidents = [
    {
      ...
      description:
        'Structural anomaly reported near the elevated transit line.',
      time: '2 min ago',
    ...
    },
    {
      ...
      description:
        'Traffic sensors report an obstruction in the westbound lane.',
      time: '24 min ago',
  ...
  const seedDatabase = database.transaction(() =>
    seedIncidents.forEach((incident) => insertIncident.run(incident)),
  )
  seedDatabase()
}
...
function toClientIncident(incident) {
  return {
    ...incident,
    position: { left: incident.left_position, top: incident.top_position },
  }
}
...
app.get('/api/incidents', (_request, response) => {
  const incidents = database
    .prepare('SELECT * FROM incidents ORDER BY id')
    .all()
...
app.put('/api/incidents/:id', (request, response) => {
  ...
  if (
    ...
    response
      .status(400)
      .json({ error: 'Status, unit, and note must be text values.' })
    return
  }
  const result = database
    .prepare('UPDATE incidents SET status = ?, unit = ?, note = ? WHERE id = ?')
    .run(status, unit, note, request.params.id)
  if (result.changes === 0) {
...
app.post('/api/briefing', async (_request, response) => {
  ...
  const incidents = database
    .prepare(
      'SELECT id, severity, location, description, status, unit, note FROM incidents ORDER BY id',
    )
    .all()
  ...
  try {
    ...
    response.json({
      briefing: result.output_text?.trim() || 'No briefing was generated.',
    })
  } catch (error) {
    console.error(
    ...
    response.status(502).json({
      error: 'Gemini could not generate a briefing. Please try again.',
    })
  }
})

app.listen(3001, () =>
  console.log('SpiderOps API listening on http://localhost:3001'),
)
