...
const initialIncidents: Incident[] = [
  {
    id: 'INC-2048',
    severity: 'Critical',
    location: 'Midtown - 8th Avenue',
    description: 'Structural anomaly reported near the elevated transit line.',
    time: '2 min ago',
    status: 'Units dispatched',
    unit: 'Unit 12 - Midtown',
    note: 'Perimeter team requested.',
    position: { left: '48%', top: '28%' },
  },
  {
    id: 'INC-2047',
    severity: 'High',
    location: 'Harbor District - Pier 14',
    description:
      'Unauthorized drone activity detected over restricted airspace.',
    time: '11 min ago',
    status: 'Assessment in progress',
    unit: 'Aerial Unit 04',
    note: 'Awaiting airspace clearance.',
  ...
  },
  {
    ...
    description: 'Traffic sensors report an obstruction in the westbound lane.',
    time: '24 min ago',
    status: 'Response unit en route',
    unit: 'Traffic Unit 07',
    note: 'Westbound lane remains restricted.',
...
function App() {
  ...
  const filteredIncidents =
    activeFilter === 'All'
      ? incidentRecords
      : incidentRecords.filter((incident) => incident.severity === activeFilter)
  ...
  const [isResponsePlanLoading, setIsResponsePlanLoading] = useState(false)
  ...
  useEffect(() => {
    async function loadIncidents() {
      try {
        ...
        console.info(
          'Local API is unavailable; displaying the built-in demo incidents.',
        )
      }
    }
    loadIncidents()
  ...
  function openEditor() {
    ...
    setDraftUnit(selectedIncident.unit)
    setDraftNote(selectedIncident.note)
    setIsEditorOpen(true)
  }
  async function saveIncident() {
    try {
      const response = await fetch(
        `http://localhost:3001/api/incidents/${selectedIncident.id}`,
        {
          method: 'PUT',
      ...
      window.alert(
        'The local API is not running. Start it with npm.cmd run server, then try again.',
      )
    }
  }
  ...
  async function generateBriefing() {
    ...
    setBriefingText('')
    try {
      const response = await fetch('http://localhost:3001/api/briefing', {
        method: 'POST',
      })
      ...
      if (!response.ok || !result.briefing)
        throw new Error(result.error || 'Could not generate briefing')
    ...
    try {
      ...
      const message =
        error instanceof TypeError
          ? 'SpiderOps API unavailable. Start the local server and try again.'
  ...
  async function generateResponsePlan() {
    ...
    setIsResponsePlanLoading(true)
    setResponsePlan(null)

    try {
      const response = await fetch(
        `http://localhost:3001/api/incidents/${selectedIncident.id}/response-plan`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ analysis: incidentAnalysis }),
        },
      )
      ...
      if (!response.ok || !result.responsePlan) {
        throw new Error(result.error || 'Could not generate response plan')
      }

      setResponsePlan(result.responsePlan)
    } catch (error) {
      const message =
        error instanceof TypeError
          ? 'SpiderOps API unavailable. Start the local server and try again.'
          : error instanceof Error
            ? error.message
            : 'Could not generate response plan'

      window.alert(message)
    } finally {
      setIsResponsePlanLoading(false)
    }
  }
    ...
    <main className="app-shell">
      <nav className="topbar" aria-label="Main navigation">
        <a
          className="brand"
          href="#overview"
          aria-label="SpiderOps dashboard home"
        >
      ...
      <section className="hero" id="overview" aria-labelledby="page-title">
        ...
        <p className="hero-copy">
          Monitor, coordinate, and resolve active incidents across the city from
          one secure command view.
        </p>

        <div className="hero-actions">
          <a className="primary-button" href="#incident-map">
            View incident map -&gt;
          </a>
      ...
      {isBriefingVisible && (
        <section className="briefing-panel" aria-labelledby="briefing-title">
          <div className="briefing-topline">
            <p className="eyebrow">GEMINI GENERATED BRIEFING</p>
          ...
          <h2 id="briefing-title">Operations Briefing</h2>

          <p className="briefing-summary">
            {isBriefingLoading
              ? 'Gemini is reviewing the current incident data...'
              : briefingText}
          </p>
          ...
          <div className="briefing-actions">
            <div>
              <span>Source</span>
              <strong>Current SQLite incident records</strong>
            </div>
          ...
          >
            <div className="editor-heading">
              <div>
                ...
                <h2 id="editor-title">{selectedIncident.id}</h2>
              </div>
            ...
            <p className="editor-location">{selectedIncident.location}</p>
            ...
            <label>
              ...
              >
                <option>Units dispatched</option>
                <option>Assessment in progress</option>
                <option>Response unit en route</option>
      ...
      >
        ...
        <div className="map-layout">
          <div
            className="city-map"
            role="group"
            aria-label="Illustrated city incident map"
          >
            <span className="district district-one">MIDTOWN</span>
            <span className="district district-two">EAST RIVER</span>
            <span className="district district-three">HARBOR</span>
            ...
            {incidentRecords.map((incident) => (
              <button
                ...
                type="button"
                key={incident.id}
                aria-label={`Select ${incident.id}, ${incident.location}`}
                onClick={() => {
            ...
            <span className="response-unit unit-one" />
            <span className="response-unit unit-two" />
          </div>
          <aside className="map-details" aria-live="polite">
            <p className="eyebrow">SELECTED INCIDENT</p>
            <div className="detail-heading">
              <span
                className={`severity-dot ${selectedIncident.severity.toLowerCase()}`}
              />
              <span
                className={`severity-label ${selectedIncident.severity.toLowerCase()}`}
              >
                {selectedIncident.severity}
              </span>
              <span>{selectedIncident.id}</span>
            </div>
            <h3>{selectedIncident.location}</h3>
            <p>{selectedIncident.description}</p>
            <div className="detail-status">
              <span>Current status</span>
              <strong>{selectedIncident.status}</strong>
              <span>Assigned unit</span>
              <strong>{selectedIncident.unit}</strong>
            </div>
            <div className="incident-actions">
              ...
              {incidentAnalysis && (
                <button
                  ...
                  disabled={isResponsePlanLoading}
                >
                  {isResponsePlanLoading
                    ? 'Planning...'
            ...
            {incidentAnalysis && (
              <div className="incident-analysis" aria-live="polite">
                <div className="analysis-heading">
                  ...
                  <span
                    className={`analysis-confidence ${incidentAnalysis.confidence.toLowerCase()}`}
                  >
                ...
                <div className="analysis-priority">
                  ...
                  <strong
                    className={`analysis-priority-${incidentAnalysis.priority.toLowerCase()}`}
                  >
            ...
            {responsePlan && (
              <div className="response-plan" aria-live="polite">
                <div className="response-plan-heading">
                  <div>
                    <p className="eyebrow">AI RESPONSE PLAN</p>
                    <h4>Recommended response</h4>
                  </div>
                  <span className="response-plan-unit">
                    {responsePlan.assignedUnit}
                  </span>
                </div>

                <div className="response-plan-objective">
                  <span>Objective</span>
                  <p>{responsePlan.objective}</p>
                </div>
                ...
                <div className="response-plan-section">
                  ...
                  <ol>
                    {responsePlan.immediateActions.map((action, index) => (
                      <li key={`${responsePlan.assignedUnit}-${index}`}>
                        {action}
                      </li>
                    ))}
                  </ol>
                ...
                <div className="response-plan-section">
                  ...
                  <p>{responsePlan.escalationTrigger}</p>
                </div>

                <div className="response-plan-section">
                  <span>Completion criteria</span>
                  <p>{responsePlan.completionCriteria}</p>
                </div>

                {responsePlan.planningNote && (
                  <div className="response-plan-note">
                    <span>Planning note</span>
                    <p>{responsePlan.planningNote}</p>
                  </div>
                )}

                <small>
                  AI-generated from the selected incident and its analysis.
                </small>
              </div>
            )}
          </aside>
      ...
      <section className="incidents-section" aria-labelledby="incidents-title">
        <div className="section-heading">
          ...
          <div className="filter-bar" aria-label="Filter incidents by severity">
            {filters.map((filter) => (
              <button
          ...
          {filteredIncidents.map((incident) => (
            <article className="incident-card" key={incident.id}>
              ...
              <div className="incident-main">
                ...
                {incident.note && (
                  <p className="operator-note">Note: {incident.note}</p>
                )}
              </div>
